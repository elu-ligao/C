...
pElemType getHead(pLinkQueue Q)
{
    ...
    memcpy(e->name, Q->front->next->data->name, sizeof(e->name)-1);
    e->sex = Q->front->next->data->sex;
    e->age = Q->front->next->data->sex;
    return e;
}
...
int enQueue(pLinkQueue Q, pElemType e)
{
    ...
    if(!q)  return -1;
    memcpy(q->data->name, e->name, strlen(e->name));
    q->data->sex = e->sex;
    q->data->age = e->age;
    Q->rear->next = q;
    Q->rear = q;
...
pElemType deQueue(pLinkQueue Q)
{
    ...
    memcpy(e->name, q->data->name, sizeof(e->name)-1);
    e->sex = q->data->sex;
    e->age = q->data->age;
    Q->front->next = q->next;
...
void queueTraverse(pLinkQueue Q)
{
    ...
    while(q)
    {
        printf("name: %s, sex: %c, age: %d\n", q->data->name, q->data->sex, q->data->age);
        q = q->next;
    }
}

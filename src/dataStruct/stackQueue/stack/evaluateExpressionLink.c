...
void evaluateExpression(char **sPriorityArr, char *sExpression)
{
    ...
    int result = 0;
    if(doExpression(sExpression, sPriorityArr, &result))
    {
        printf("result = %d\n", result);
    }
...
bool doExpression(char *sExpression, char **sPriorityArr, int *rst)
{
    
    pStackLink OPTR = initStack();
    pStackLink OPND = initStack();
    pElemType e = NULL;

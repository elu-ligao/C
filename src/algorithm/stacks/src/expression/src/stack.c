/*
 * @Author       : elu
 * @Date         : 2022-01-13 16:13:45
 * @LastEditTime : 2022-01-14 16:51:21
...
int Push(PtrElem X, Stack S)
{
    if(IsFull(S))
    {
        printf("Full Stack. X = %.2f[%s] Push fail.\n", X->data, X->sdata);
        return -1;
    }
    S->TopOfStack += 1;
    CopyElemData(&S->Array[S->TopOfStack], X);
    // printf("Push X = %.2f Success.\n", X->data);
    return 0;
}
...
/**
 * @descripttion:   新建Element 空结点
 * @return {*}
 */
PtrElem NewElement()
{
    return (PtrElem)calloc(1, sizeof(ElemType));
}
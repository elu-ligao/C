/*
 * @Author       : elu
 * @Date         : 2021-11-29 23:00:04
 * @LastEditTime : 2021-11-29 23:05:35
...
int getData(pLinkQueue Q, char sex, char *prefix);

int main(int argc, char *argv[])
{

    int rc = initLinkQueueHandle();
    if(rc)
    {
        return -1;
    }
    ...
    pLinkQueue FQ = initQueueFunc();
    if(!FQ) return -1;
    pLinkQueue MQ = initQueueFunc();
    if(!MQ) return -1;
...
int getData(pLinkQueue Q, char sex, char *prefix)
{
    ...
    for(; i<num; ++i)
    {
        ...
        e->age = rand()%60+10;
        e->sex = sex;
        sprintf(e->name, "%s%03d", prefix, rand()%1000);
